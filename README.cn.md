# open-design

[English version](./README.md)

🎨 Best DeepSeek Harness Design Plugin. The open-source Claude Design alternative. 🖥️ Local-first desktop app. 🖼️ Your coding agent becomes the design engine: prototypes, landing pages, dashboards, slides, images & video — real files, HTML/PDF/PPTX/MP4 export. 🤖 Claude Code / Codex / Cursor / DeepSeek Harness / OpenCode & 20+ CLIs via BYOK.

[![x-cmd/install — open-design Code Quality Monitoring Repo Card](https://x-cmd.com/repo-card/open-design.svg?lang=zh)](https://x-cmd.com/install/open-design)

## 安装

```sh
x install open-design
```

## 代码洞察

合计: **2,244,881** 行代码（覆盖前 5 种语言、共 **9494** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| TypeScript | 1,040,748 | 115,592 | 78,586 | 3585 |
| Json | 441,429 | 0 | 58 | 1850 |
| Tsx | 407,550 | 51,393 | 33,258 | 1056 |
| Html | 162,443 | 3,265 | 4,503 | 2467 |
| Css | 115,429 | 20,666 | 6,021 | 536 |

## 源代码

- **上游仓库**: <https://github.com/nexu-io/open-design>
- **官网**: <https://open-design.ai>
- **许可证**: Apache-2.0

## 发布

- **最新版本**: `open-design-v0.24.1` (2026-09-24)
- **最近提交**: 2026-09-24
- **Release 含资产**: 3 个

## 流行度

- **Star**: 98,116 · **Fork**: 11,385 · **开放 issue**: 2,079 · **贡献者**: 455

## 累计统计

- **发布数**: 38 · **已合并 PR**: 3486 · **开放 PR**: 603 · **已关闭 issue**: 1560 · **开放 issue**: 519 · **提交数**: 3700

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-27 | 7 | 371 | 243 | 29 | 143 | 239 |
| last60d | 2026-07-28 | 17 | 990 | 399 | 154 | 273 | 678 |
| 90d | 2026-06-28 | 24 | 1587 | 538 | 322 | 358 | 1195 |
| last180d | 2026-03-30 | 38 | 3486 | 603 | 1560 | 519 | 3556 |
| 360d | 2025-10-01 | 38 | 3486 | 603 | 1560 | 519 | 3556 |
| last720d | 2024-10-06 | 38 | 3486 | 603 | 1560 | 519 | 3700 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [open-design-0.24.1-mac-arm64.dmg](https://github.com/nexu-io/open-design/releases/download/open-design-v0.24.1/open-design-0.24.1-mac-arm64.dmg) | 382.2 MiB | `other` |
| [open-design-0.24.1-mac-x64.dmg](https://github.com/nexu-io/open-design/releases/download/open-design-v0.24.1/open-design-0.24.1-mac-x64.dmg) | 401.4 MiB | `other` |
| [open-design-0.24.1-win-x64-setup.exe](https://github.com/nexu-io/open-design/releases/download/open-design-v0.24.1/open-design-0.24.1-win-x64-setup.exe) | 417.9 MiB | `other` |

## 改进这些数据

open-design 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `open-design` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/open-design.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260926.yml` · 2026-09-26T05:25:48Z._
